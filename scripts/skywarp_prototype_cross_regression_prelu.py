...
from tensorflow.keras.layers import (Layer, Input, Conv1D, ReLU, PReLU, MaxPooling1D, GlobalAveragePooling1D, Conv1DTranspose, Concatenate, Dense, MultiHeadAttention, LayerNormalization, Dropout, UpSampling1D)
...
def create_denoising_layers():
    ...
    x = Conv1D(filters=64, kernel_size=8, padding='same', kernel_initializer=HeNormal())(inputs)
    x = PReLU()(x)
    x = MaxPooling1D(pool_size=8, strides=8)(x)
    x = Conv1D(filters=32, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = Conv1D(filters=32, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = MaxPooling1D(pool_size=8, strides=8)(x)
    x = Conv1D(filters=16, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = Conv1D(filters=16, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    encoded = Conv1D(filters=16, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    encoded = PReLU()(encoded)

    # Decoder part
    x = Conv1DTranspose(filters=16, kernel_size=8, padding='same', kernel_initializer=HeNormal())(encoded)
    x = PReLU()(x)
    x = Conv1DTranspose(filters=16, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = UpSampling1D(size=8)(x)
    x = Conv1DTranspose(filters=32, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = Conv1DTranspose(filters=32, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
    x = UpSampling1D(size=8)(x)
    x = Conv1DTranspose(filters=64, kernel_size=8, padding='same', kernel_initializer=HeNormal())(x)
    x = PReLU()(x)
...
def build_starscream(input_shape, config):
    ...
    output = Dense(28, activation='linear', kernel_initializer=HeNormal())(x)
